...
function workspaceStudioCommonPrompt_() {
  return [
    ...
    '- 次のJSONだけを返してください: {"body":"投稿本文","tags":["タグ1","タグ2"],"sourceLabel":"参照テーマ（機密を含めない）","sourceUrl":"Google Workspace内のURL。安全に示せない場合は空文字"}',
    '',
    ...
    '- 次のJSONだけを返してください: {"body":"返信本文"}',
    '',
    'typeが"reply-choice"の場合:',
    '- candidatesの中から、personaの視点で最も有意義に議論を進められる投稿を1件選んでください。',
    '- 候補は、問い、違和感、未完了の印、時間経過、最近繰り返されたテーマをもとに選ばれています。',
    '- 既存返信と重複せず、補足、具体例、反証、問い直し、または次の一手につながる返信にしてください。',
    '- 次のJSONだけを返してください。targetPostIdには候補のidをそのまま入れてください: {"targetPostId":"投稿ID","body":"返信本文"}',
    '',
