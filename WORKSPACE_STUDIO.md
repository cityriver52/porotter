...
Gemini用プロンプトは動作ごとに自動生成されます。新規投稿では過去7日程度に更新されたDriveファイル、最近受信したメール、最近届いたChatメッセージから業務のヒントを探します。返信でも、対象の投稿やユーザー返信に加えて、関連する最近のWorkspace情報を参照し、単なる称賛や要約で終わらない240文字以内の返答を作ります。
